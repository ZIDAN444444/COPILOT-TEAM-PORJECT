...
Temuan penting:
  ...
  - Endpoint /detail    → AKTIF  untuk dokumen panduan (is_api_doc=False)

Jadi script ini:
  * Mengunduh JSON lengkap untuk semua API Reference docs via /api_meta
  * Mengunduh JSON lengkap untuk semua Guide docs via /detail
  * Fallback ke metadata jika endpoint tetap gagal
  * Penamaan file & folder menggunakan field "name" dari tree

Cara pakai:
    python partner_tiktokshop_com.py
...
OUTPUT_DIR      = Path("docs/partner_tiktokshop_com")
DELAY_SECONDS   = 0.35
TIMEOUT_SECONDS = 30
MAX_RETRIES     = 3
...
def fetch_api_meta(doc_path: str):
    """Unduh API Reference doc via endpoint /api_meta (is_api_doc=True)."""
...
def fetch_api_doc(doc_path: str):
    """Unduh Guide/panduan doc via endpoint /detail (is_api_doc=False)."""
...
def process_node(node, parent_dir: Path, depth: int = 0):
    ...
    # ── Nama file selalu dari "name" ─────────────────────────────────────────
    filename  = safe_name(name) + ".json"
    file_path = parent_dir / filename

    if is_api:
        # ── API Reference: unduh via /api_meta ───────────────────────────────
        stats["api_total"] += 1
        ...
        # ── Panduan/Guide: unduh via /detail ─────────────────────────────────
        stats["doc_total"] += 1
        ...
        if detail:
            ...
            # Fallback: simpan metadata saja jika endpoint tetap gagal
            save_json(file_path, {
...
def main():
    ...
    print(f"    ✓ Berhasil diunduh: {stats['api_success']}")
    print(f"    ✔ Resume (skip)   : {stats['api_skipped']}")
    print(f"    ✗ Gagal           : {stats['api_failed']}")
    print(f"  Guide/panduan docs  : {stats['doc_total']} total")
    print(f"    ✓ Berhasil diunduh: {stats['doc_success']}")
